...
<div id="WidgetCardapio_${instanceId}"
			...
			<div class="media media-meal">
			    ...
			    <div class="media-body">
			        <h2 class="media-heading">
						<small>{{dayOfTheWeek}} {{#isToday}}<span class="label label-info">${i18n.getTranslation('kit_cardapio.today')}</span>{{/isToday}}</small><br>
						{{mealTitle}}
					</h2>
					{{mealDescMin}}
			    </div>
	...
	<script type="text/template" class="template-course-detail">
		<div class="media media-meal">
		    ...
		    <div class="media-body">
		        <h2 class="media-heading">
					{{#isToday}}<small><span class="label label-info">${i18n.getTranslation('kit_cardapio.today')}</span></small><br>{{/isToday}}
					{{mealTitle}}
				</h2>
				{{mealDesc}}
		    </div>
