<div id="ConfiguracaoValidadeDocumentos_${instanceId}" class="super-widget wcm-widget-class fluig-style-guide" 
	data-params="ConfiguracaoValidadeDocumentos.instance()">

	<h2 class="page-header"> 
		<span class="fluigicon fluigicon-replace-file"></span>
		Configuração de Validade de Documentos
	</h2>
...
<script type="text/javascript">
	...
	var fillFolderDescription_${instanceId} = function(item) {
		...
		if (item.parent != null) {
			folderDescription = fillFolderDescription_${instanceId}(item.parent) + folderDescription;
		} 
		...
		for (var i=0; i<all_data_${instanceId}.length; i++) {
			...
			if (!item.folder) {
				...
				if (canAdd) {
					...
					item.folderTitle = fillFolderDescription_${instanceId}(item.parent);
