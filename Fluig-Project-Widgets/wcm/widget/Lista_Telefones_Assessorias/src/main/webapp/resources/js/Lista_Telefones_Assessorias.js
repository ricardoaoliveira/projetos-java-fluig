var ListaTelefonesAssessorias = SuperWidget.extend({
	...
	searchData: function() {
		...
		var fields = new Array("id", "nm", "fone_sac", "fone");
		var sortingFields = new Array("nm");
				
		var dataset = DatasetFactory.getDataset("ds_telefones_assessorias", fields, constraints, sortingFields);
    ...
    initTable: function() {
    	...
    	for (var i=0; i<data.content.length; i++) {
    		...
    		if (data.content[i].fone_sac != null && data.content[i].fone_sac.length > 0) {
				data.content[i].foneSacLink = this.strToLinkTel( data.content[i].fone_sac );
			}
    	...
    	var template = "{{#content}}<tr class='{{td_class}}'>" +
    		...
    		"<td class='{{td_class}}'><a href='{{foneSacLink}}'>{{fone_sac}}</a></td>" +
