var SlideShowCardapio = SuperWidget.extend({
	instanceId: null,
	widgetVersion: null,
	sourceType: null,
	...
	init: function () {
		...
		if (that.isEditMode) {
			...
			that.processTemplate('template_slideshow', {}, '#divSlideShowCardapioshow' + that.instanceId, function () {
				that.viewMode();
			});
		}
	},
		...
		if (that.sourceType === that.mapAttrs.TYPE_FLUIGDIR) {
			...
			that.getInstagramImageReader(function (response) {
				if (response) {
					...
					that.showMessage('', 'danger', '${i18n.getTranslation("slide_show_cardapio.error.noresponse")}');
				}
			});
		}
	},
		...
		for (var i = 0; i < len; i++) {
			...
			if (item.mimetype && that.validateMimeType(item)) {
				image = {
					...
					templateTitle: that.getFluigFileDescription(item),
				};
				images.push(image);
			}
		}
	...
	buildSlideShow: function (images) {
		...
		if (images && images.length) {
			settings = {
				id: 'divSlideShowCardapioshow' + this.instanceId,
				images: images,
				autoSize: this.autoSize,
				resize: this.resize,
				interval: this.interval,
				teamplate: ".template_slideshow"
			};
			FLUIGC.carousel(photoList, settings);
			
			//FLUIGC.carousel('divSlideShowCardapioshow' + this.instanceId, settings);
			
			$('#divSlideShowCardapioshow' + this.instanceId).on('slid.bs.carousel', function (e) {
		        
		    }); 
			
			$('#divSlideShowCardapioshow' + this.instanceId).on('slide.bs.carousel', function () {
		        
		    });
			
			
		} else {
			this.showMessage('', 'warning', '${i18n.getTranslation("slide_show_cardapio.error.nodatatodisplay")}');
		}
	},
	...
	parseError: function (response) {
		...
		switch (response.meta.error_type) {
			case that.mapAttrs.ERROR_TYPE_API_NOT_ALLOWED:
				return '${i18n.getTranslation("slide_show_cardapio.error.apinotallowed")}';
			case that.mapAttrs.ERROR_TYPE_API_INVALID_CLIENT:
				return '${i18n.getTranslation("slide_show_cardapio.error.invalidclientid")}';
			default:
	...
	displayNoDataFoundMessage: function () {
		this.showMessage('', 'info', '${i18n.getTranslation("slide_show_cardapio.error.nodatatodisplay")}');
	},
	...
	getInstagramImageReader: function (callback) {
		...
		that.serviceInstagramUtil(urlInvokeService, function (err, data) {
			if (err) {
				FLUIGC.toast({
					message: '${i18n.getTranslation("slide_show_cardapio.error.invalidaccount")}',
					type: 'danger'
				});
				return false;
			}
	...
	save: function (preferences) {
		...
		if (that.sourceType === that.mapAttrs.TYPE_FLUIGDIR && preferences.fluigDirectoryName === ''
			...
			that.showMessageError('', '${i18n.getTranslation("slide_show_cardapio.edit.error.atleastone")}');
		} else {
	...
	chooseSourceType: function (type) {
		...
		if (type === this.mapAttrs.TYPE_FLUIGDIR) {
			$optionButton.text('${i18n.getTranslation("slide_show_cardapio.source.fluigdir")}' + ' ');
			displayFluigDirData = '';
			displayInstagramData = 'none';
			$("#showImageTitleEdit_" + this.instanceId).removeClass('fs-display-none');
		} else {
			$optionButton.text('${i18n.getTranslation("slide_show_cardapio.source.instagram")}' + ' ');
	...
	chooseDirectory: function () {
		...
		var cfg = {
			...
			width: 750,
			height: 500,
			title: '${i18n.getTranslation("slide_show_cardapio.edit.copyPhisicalFile.title")}',
			callBack: function () {
				ECM.findDocument.getDocuments(0, '1-2-8');
			},
			customButtons: new Array('${i18n.getTranslation("slide_show_cardapio.edit.select")}')
		};
		...
		ECM.findDocument.panel.bind('panel-button-0', function () {
			if (ECM.findDocument.dataTable.selectedRows.length === 0) {
				WCMC.messageWarn('${i18n.getTranslation("slide_show_cardapio.edit.error.atleastone")}' + '.');
				return;
			}
			...
			if (ECM.findDocument.dataTable.selectedRows.length === 1) {
				...
				WCMC.messageWarn('${i18n.getTranslation("slide_show_cardapio.edit.error.justone")}' + '.');
				return;
